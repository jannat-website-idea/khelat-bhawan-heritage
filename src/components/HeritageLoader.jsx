...
export default function HeritageLoader({ onComplete, onReveal, lang }) {
  ...
  useEffect(() => {
    ...
    const duration = reduced ? 400 : 3200;
    ...
    const tick = (now) => {
      ...
      if (sequence >= 1 && canOpen && !finishing) {
        ...
        holdTimer = setTimeout(() => {
          ...
          onReveal?.(true);
          setLeaving(true);
          exitTimer = setTimeout(() => onComplete(false), reduced ? 0 : 900);
        }, reduced ? 0 : 250);
      } else {
  ...
  const titleOpacity = Math.min(1, 0.2 + (progress / 100) * 0.8);
  const titleTracking = `${0.02 + (progress / 100) * 0.08}em`;
  ...
  return (
    ...
    >
      <div className="palace-entrance__portrait" aria-hidden="true">
      ...
      <div className="palace-entrance__composition">
        <p className="palace-entrance__label">
        ...
        <h2
          style={{
            ...
            transform: `scale(${0.96 + (progress / 100) * 0.04})`,
            transition: 'opacity 0.4s ease-out, letter-spacing 0.4s ease-out, transform 0.4s ease-out'
          }}
        >
          {bn ? 'খেলাৎ' : 'Khelat'}
          <em>{bn ? 'ভবন' : 'Bhawan'}</em>
        ...
        <span className="palace-entrance__caption">
          {bn ? 'এক জীবন্ত উত্তরাধিকার' : 'A living legacy'}
        </span>
      </div>
      ...
      <footer>
        <div className="palace-entrance__progress">
          <div>
            <span>{bn ? 'বাড়িতে প্রবেশ' : 'Entering the house'}</span>
          ...
          <progress
            aria-label={bn ? 'প্রবেশের অগ্রগতি' : 'Entrance sequence progress'}
            value={progress}
            max="100"
          />
        </div>
