    ...
    >
      ...
      <div className="flex items-center justify-between z-10 w-full" onClick={(e) => e.stopPropagation()}>
        <div>
          <span className="text-[10px] md:text-xs tracking-[0.3em] uppercase text-rose-gold font-body">
            Khelat Bhavan Rajbari · Visual Archive
          </span>
      ...
      <div className="relative flex-1 flex items-center justify-center my-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        {hasPrev && (
          <button
            onClick={onPrev}
            className="absolute left-2 md:left-6 w-12 h-12 rounded-full bg-navy/80 hover:bg-accent text-white flex items-center justify-center transition-all border border-white/10 backdrop-blur-sm"
        ...
        {hasNext && (
          <button
            onClick={onNext}
            className="absolute right-2 md:right-6 w-12 h-12 rounded-full bg-navy/80 hover:bg-accent text-white flex items-center justify-center transition-all border border-white/10 backdrop-blur-sm"
      ...
      {/* Bottom Description */}
      <div className="max-w-3xl mx-auto text-center px-4" onClick={(e) => e.stopPropagation()}>
        <p className="text-xs md:text-sm text-primary-foreground/75 font-body leading-relaxed">
          {item.desc}
        </p>
      </div>
    </div>
